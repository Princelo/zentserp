...
class Trial_Product extends MY_Controller {
    ...
    public function listpage_admin($offset = 0)
    {
        ...
        )
        {
            ...
            $data = array();
            $config['base_url'] = base_url()."trial_product/listpage_admin/";
            if (count($_GET) > 0) $config['suffix'] = '?' . http_build_query($_GET, '', "&");
            $config['first_url'] = $config['base_url'].'?'.http_build_query($_GET);
            $where = '';
            $where .= ' and p.is_valid = true ';
            $where .= $this->__get_search_str($search, $price_low, $price_high, $category);
            $config['total_rows'] = $this->MTrialProduct->intGetProductsCount($where);
            $config['per_page'] = 30;
            ...
            //$where = '';
            $order = '';
            $data['products'] = $this->MTrialProduct->objGetProductList($where, $order, $limit);
            ...
            if (count($_GET) > 0) $config['suffix'] = '?' . http_build_query($_GET, '', "&");
            $config['first_url'] = $config['base_url'].'?'.http_build_query($_GET);
            $config['total_rows'] = $this->MTrialProduct->intGetProductsCount(' and p.is_valid = true ');
            $config['per_page'] = 30;
            $this->pagination->initialize($config);
            $data['page'] = $this->pagination->create_links();
            $limit = '';
    ...
    public function listpage_admin_invalid($offset = 0)
    {
        ...
        )
        {
            ...
            $category = $this->input->get('category', true);
            $data = array();
            $config['base_url'] = base_url()."trial_product/listpage_admin_invalid/";
            if (count($_GET) > 0) $config['suffix'] = '?' . http_build_query($_GET, '', "&");
            $config['first_url'] = $config['base_url'].'?'.http_build_query($_GET);
            $where = '';
            ...
            if (count($_GET) > 0) $config['suffix'] = '?' . http_build_query($_GET, '', "&");
            $config['first_url'] = $config['base_url'].'?'.http_build_query($_GET);
            $config['total_rows'] = $this->MTrialProduct->intGetProductsCount(' and p.is_valid = false ');
            $config['per_page'] = 30;
            $this->pagination->initialize($config);
            $data['page'] = $this->pagination->create_links();
            $limit = '';
    ...
    public function listpage($offset = 0)
    {
        ...
        )
        {
            ...
            $category = $this->input->get('category', true);
            $data = array();
            $config['base_url'] = base_url()."trial_product/listpage/";
            if (count($_GET) > 0) $config['suffix'] = '?' . http_build_query($_GET, '', "&");
            $config['first_url'] = $config['base_url'].'?'.http_build_query($_GET);
            $where = '';
            $where .= ' and p.is_valid = true ';
    ...
    public function details_admin($product_id)
    {
        if($this->session->userdata('role') != 'admin')
            exit('You are not the admin.');
        $data = array();
        $data['v'] = $this->MTrialProduct->objGetProductInfo($product_id);
        $config = array(
            array(
                'field'   => 'title',
                'label'   => '产品名称',
        ...
        if(isset($_POST) && !empty($_POST))
        {
            if ($this->form_validation->run() == FALSE)
            {
                ...
                if($this->MTrialProduct->update($main_data, $product_id))
                {
                    $this->session->set_flashdata('flashdata', '产品更改成功');
                }else{
                    $this->session->set_flashdata('flashdata', '产品更改失败');
                }
                redirect('trial_product/details_admin/'.$product_id);
    ...
    public function add($error = '')
    {
        ...
        if(isset($_POST) && !empty($_POST))
        {
            if ($this->form_validation->run() == FALSE)
            {
                ...
                $result = $this->MTrialProduct->add($main_data);
                if($result){
                    $this->session->set_flashdata('flashdata', '产品添加成功');
                    redirect('trial_product/add');
                }
                else{
    ...
    private function __get_search_str($search = '', $price_low = '', $price_high = '', $category = null)
    {
        ...
        if($search != '' && $price_low != '' && $price_high != '')
        {
            $where .= " and (p.title like '%{$search}%' or p.feature like '%{$search}%' or
                            (cast(p.price as numeric) between {$price_low} and {$price_high} )
                            ) ";
        ...
        {
            $where .= " and (p.title like '%{$search}%' or p.feature like '%{$search}%' or
                            (cast(p.price as numeric) > {$price_low} )
                            ) ";
        }elseif($search != '' && $price_low == '' && $price_high != '')
        {
            $where .= " and (p.title like '%{$search}%' or p.feature like '%{$search}%' or
                            (cast(p.price as numeric) < {$price_high} )
                            ) ";
        }elseif($search == '' && $price_low != '' && $price_high != '')
        {
            $where .= " and (cast(p.price as numeric) between {$price_low} and {$price_high}) ";
        }elseif($search == '' && $price_low != '' && $price_high == '')
        {
            $where .= " and (cast(p.price as numeric) > {$price_low} )";
        }elseif($search == '' && $price_low == '' && $price_high != '')
        {
            $where .= " and (cast(p.price as numeric) < {$price_high} )";
        }
